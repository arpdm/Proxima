...
class ManufacturingSectorBuilder(ComponentBuilder):
    ...
    def build(
        ...
        print(f"✅ Configured manufacturing sector: {len(config['isru_extractors'])} extractors, {len(config['isru_generators'])} generators")
        return config
...
def build_world_system_config(world_system_id: str, experiment_id: str, db: ProximaDB) -> dict:
    ...
    config["agents_config"]["energy"] = energy_builder.build(
        components_dict.get("energy", [])
    )
    
    science_builder = ScienceSectorBuilder(component_templates)
    config["agents_config"]["science"] = science_builder.build(
        components_dict.get("science", [])
    )
    ...
    config["agents_config"]["manufacturing"] = manufacturing_builder.build(
        components_dict.get("manufacturing", []),
        world_system.get("initial_stocks", {})
    )
    ...
    config["agents_config"]["transportation"] = transportation_builder.build(
        components_dict.get("transportation", [])
    )
...
_configure_manufacturing_sector = lambda components, templates, world_system: ManufacturingSectorBuilder(templates).build(components, world_system.get("initial_stocks", {}))
_configure_equipment_manufacturing_sector = lambda components, templates: EquipmentManufacturingSectorBuilder(templates).build(components)
_configure_transportation_sector = lambda components, templates: TransportationSectorBuilder(templates).build(components)
